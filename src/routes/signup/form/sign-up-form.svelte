<script lang="ts">
  ...
  import { formSchema, schemaStep1, schemaStep2, schemaStep3, type FormSchema } from './sign-up-form-schema';
  import SuperDebug, {
    type SuperValidated,
    type Infer,
    superForm,
  ...
  const validateEmail = async (email: string) => {
    ...
    if (validFormat) {
      identIsAvailable = await myUserContext.isUserIdentAvailable(email, UserIdentType.email) || false
    }
