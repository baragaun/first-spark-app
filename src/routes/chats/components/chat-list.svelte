<script lang="ts">
  ...
  const getRecipientName = (channel: Channel) => {
    ...
    console.log('all participants:', channel.participants);
    const recipientParticipant = channel.participants.find(
      (participant) => participant.userId !== currentUserId,
    );

    console.log('recipientParticipant', recipientParticipant);
...
<div class="space-y-2">
  ...
  {:else}
    {#each channels as channel}
      ...
      >
        <a href={`/chats/${channel.id}`} class="flex items-center gap-4">
