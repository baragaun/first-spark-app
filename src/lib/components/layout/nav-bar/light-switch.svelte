<script lang="ts">
  ...
  const themeOptions = [
    ...
    { value: 'kcu', label: m['light_switch.kcu'](), action: () => setMode('kcu') },
  ];
</script>
    ...
    <Tooltip.Provider>
      <Tooltip.Root>
        <Tooltip.Trigger>
          ...
          >
            <Sun
              class="h-[1.2rem] w-[1.2rem] rotate-0 scale-100 transition-all dark:-rotate-90 dark:scale-0 kcu:-rotate-90 kcu:scale-0"
            />
            <Moon
              class="absolute h-[1.2rem] w-[1.2rem] rotate-90 scale-0 transition-all dark:rotate-0 dark:scale-100 kcu:rotate-90 kcu:scale-0"
            />
            <Palette
              class="absolute h-[1.2rem] w-[1.2rem] rotate-90 scale-0 transition-all dark:rotate-90 dark:scale-0 kcu:rotate-0 kcu:scale-100"
            />
            {#if !iconButton}
